...
function loadAllTemplateJsons() {
    const repoRoot = node_path_1.default.resolve(__dirname, '../../../');
    const dir = node_path_1.default.join(repoRoot, 'sample_data/successful_data/iauditor_templates');
    const results = [];
    ...
    if (node_fs_1.default.existsSync(dir)) {
        ...
        for (const file of files) {
            try {
                const json = JSON.parse(node_fs_1.default.readFileSync(file, 'utf-8'));
                if (!json.template_id) {
                    const filename = node_path_1.default.basename(file, '.json');
                ...
                if (!json.name) {
                    json.name = node_path_1.default.basename(file, '.json');
                }
                if (!json.description) {
...
async function main() {
    ...
    const findTemplate = (templateId) => createdTemplates.find(t => t.templateId === templateId);
